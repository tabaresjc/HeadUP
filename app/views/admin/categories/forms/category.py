...
class CategoryForm(FlaskForm):
    name = TextField(_lg('CATEGORY_NAME'), [validators.Length(min=3, max=100), validators.InputRequired()])
    slug = TextField(_lg('CATEGORY_SLUG'), [validators.Length(min=0, max=200)])
    description = TextAreaField(_lg('CATEGORY_DESC'), [validators.Length(min=3, max=1024), validators.InputRequired()])
